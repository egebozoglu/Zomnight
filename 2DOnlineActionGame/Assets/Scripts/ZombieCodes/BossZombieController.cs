...
public class BossZombieController : MonoBehaviour
{
    ...
    void Update()
    {
        if (health == 0)
        {
            BossZombieStats bossZombieStat = new BossZombieStats(killerID);
            GameManager.instance.bossZombieStats.Add(bossZombieStat);
            //ZombieStats zombieStat2 = new ZombieStats(killerID);
            //GameManager.instance.zombieStats.Add(zombieStat2);
            //ZombieStats zombieStat3 = new ZombieStats(killerID);
            //GameManager.instance.zombieStats.Add(zombieStat3);
    ...
    public void Movement()
    {
        if (attacking)
        {
            ...
            if (attackingSpriteNo >= attackingSprites.Count)
            {
                attackingSpriteNo = 0;
            }
        }
        else
        {
            ...
            if (walkingSpriteNo >= walkingSprites.Count)
            {
                walkingSpriteNo = 0;
            }
        }
